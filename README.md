...
OMO는 해외 체류를 준비하는 사용자가 자연어로 자신에게 맞는 도시를 찾고, 도시 비교부터 예산과 준비 로드맵까지 한곳에서 관리할 수 있도록 돕는 서비스입니다.
...
- 자연어 검색 조건 분석 및 도시 추천
- 최초 검색과 이어 묻기를 지원하는 AI 검색 세션
- 비동기 브리핑 생성 및 `taskId` 기반 상태·결과 조회
- 조건에 맞는 도시가 없을 때 조건 완화 제안
- 오래된 AI 검색 세션 정기 정리

### 도시 탐색 및 비교

- 국가·도시·체류 목적 목록 조회
- 예산, 치안, 언어, 비자, 주거 및 인프라 조건 기반 도시 검색
- 도시 핵심 요약, 장단점, 통계, 후기 및 참고 자료 조회
- 관심 도시 저장 및 복수 도시 비교

### 준비 로드맵

- 도시와 체류 목적에 맞는 로드맵 자동 생성
- 템플릿 기반 태스크, 선행 관계 및 준비 문서 생성
- 선행 태스크 완료 여부에 따른 태스크 상태 계산
- 출국일 기준 일정과 D-Day 관리
- 준비 문서 체크 및 로드맵 진행률 계산
- 항목별 예산 관리

### 회원 및 인증

- 이메일 회원가입·로그인 및 이메일 인증
- JWT Access/Refresh Token 발급과 재발급
- Redis 기반 Refresh Token 관리 및 로그아웃 토큰 차단
- Google OAuth 회원가입·로그인·계정 연결
- OAuth `state`와 일회용 로그인 티켓을 이용한 SPA 인증 연동
- 회원 정보, 프로필 이미지 및 알림 설정 관리

### 파일 및 문의

- Presigned URL을 이용한 클라이언트-S3 직접 업로드
- 업로드 전 파일 확장자·MIME 타입·크기 검증
- 업로드 후 S3 객체 메타데이터 재검증
- 프로필 이미지 교체 및 기존 객체 정리
- 업로드 세션 기반 문의 첨부파일 검증과 영구 경로 이동
